export default {
  ...
  poolHint:
    "为同一提供商添加多个 API Key,以便自动轮换和负载均衡。Mercury 会在它们之间轮流使用。",
  add: "添加",
  remove: "移除",
  keyLabel: "密钥",
  empty: "(空)",
  dataSection: "数据",
  dataHint: "导出或导入你的 Mercury 配置、会话、技能和记忆。",
  backingUp: "正在备份...",
  exportBackup: "导出备份",
  importing: "正在导入...",
  importBackup: "导入备份",
  ...
  migrationDesc:
    "在 <code>{{path}}</code> 发现 OpenClaw。你可以将配置、API Key、会话和技能迁移到 Mercury。",
  migrationDismiss: "不再显示",
  migrating: "迁移中...",
  migrateToHermes: "迁移到 Mercury",
  skip: "跳过",
  ...
  modeLocal: "本地",
  modeRemote: "远程",
  modeLocalHint: "使用本机安装的 Mercury",
  modeRemoteHint: "连接到网络或云服务器上的 Mercury API",
  remoteUrl: "远程服务器地址",
  remoteUrlHint:
    "Mercury API 服务器地址（需开放 /health 和 /v1/chat/completions）",
  remoteApiKey: "API 密钥",
  remoteApiKeyHint:
  ...
  serverConfigHint:
    "你已连接到远程 Mercury 服务器。模型选择、提供商 API Key 和凭据均在服务器的 <code>~/.hermes/.env</code> 和 <code>config.yaml</code> 中管理。请在主机上编辑（例如 <code>docker exec -it hermes vi /opt/data/.env</code>）然后重启容器。",
  connectionMode: "模式",
